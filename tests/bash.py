...
semantic_code_checksum = _get_semantic(executor_code, bytes.fromhex(executor_code_checksum))
bashcode = '(ls $a; date) > RESULT'
result = _run_transformer(
    semantic_code_checksum, 
    executor_code_buffer, bytes.fromhex(executor_code_checksum), 
    signature=None, 
    meta=None,
    args=[],
    kwargs={
